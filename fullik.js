...
Fullik.lerp = function ( a, b, percent ) {

	return a + ( b - a ) * percent;

};
Fullik.randRange = function ( min, max ) {

	return Fullik.lerp( min, max, Math.random() );

};
Fullik.randRangeInt = function ( min, max, n ) {

	return Fullik.lerp( min, max, Math.random() ).toFixed( n || 0 ) * 1;

};
Fullik.nearEquals = function ( a, b, t ) {

	return Math.abs( a - b ) <= t ? true : false;

};
Fullik.sign = function ( v ) {

	return v >= 0 ? 1 : - 1;

};

Fullik.radtodeg = function ( v ) {

	return v * Fullik.todeg;

};
Fullik.degtorad = function ( v ) {

	return v * Fullik.torad;

};
//Return the co-tangent of an angle specified in radians.
Fullik.cot = function ( a ) {
...
Fullik.rotateXDegs = function ( v, angleDegs ) {

	return Fullik.rotateXRads( v, angleDegs * Fullik.torad );

};
Fullik.rotateYDegs = function ( v, angleDegs ) {

	return Fullik.rotateYRads( v, angleDegs * Fullik.torad );

};
Fullik.rotateZDegs = function ( v, angleDegs ) {

	return Fullik.rotateZRads( v, angleDegs * Fullik.torad );
...
Fullik.Joint.prototype = {
	...
	validateAngle: function ( angle ) {
		...
		if ( angle < Fullik.MIN_ANGLE_DEGS ) {

			angle = Fullik.MIN_ANGLE_DEGS; console.log( '! min angle is ' + Fullik.MIN_ANGLE_DEGS );

		}
		if ( angle > Fullik.MAX_ANGLE_DEGS ) {

			angle = Fullik.MAX_ANGLE_DEGS; console.log( '! max angle is ' + Fullik.MAX_ANGLE_DEGS );
...
Fullik.Chain.prototype = {
	...
	setBaseboneRelativeReferenceConstraintUV: function ( constraintUV ) {
